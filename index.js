...
class Car {
	...
	drive(distance) {
		...
		if (maxDistance < distance) {
			...
			return 'I ran out of gas after ' + maxDistance + ' miles.';
		}
	}
}
...
class Lambdasian {
	constructor(args) {
		(this.name = args.name), (this.age = args.age), (this.location = args.location);
	}
	speak() {
...
class Instructor extends Lambdasian {
	constructor(args) {
		super(args);
		(this.specialty = args.specialty), (this.favLanguage = args.favLanguage), (this.catchPhrase = args.catchPhrase);
	}
	demo(subject) {
...
class Student extends Lambdasian {
	...
	PRAssignment(subject) {
		return `${this.name} has submnitted a PR for ${subject}`;
	}
	sprintChallenge(subject) {
		return `${this.name} has begun sprint challenge on ${subject}`;
	}
	graduate() {
		this.grade > 70 ? `${this.name} can graduate!` : `Need to grade more work for ${this.name}.`;
	}
}
...
class ProjectManager extends Instructor {
	constructor(args) {
		super(args);
		(this.gradClassName = args.gradClassName), (this.favInstructor = args.favInstructor);
	}
	standUp(channel) {
